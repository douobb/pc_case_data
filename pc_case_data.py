...
allBrands = ["Fractal Design","COUGAR","曜越","Apexgaming","SAMA","銀欣","台鼎環球","樹昌","B-Team","聯力","酷碼","Phanteks","全漢","darkFlash","abee","華碩","Montech","視博通","微星","技嘉","亞碩","旋剛","1st Player","Antec","BitFenix","HYTE","be quiet!","NZXT","賽德斯","喬思伯","迎廣","SSUPD","XPG","保銳","海盜船","海韻","i-CoolTw","DEEPCOOL"]
...
for i in range(len(titles)):
  ...
  if("華碩 TUF Gaming GT502" in titles[i] or "微星 MAG PANO M100R PZ" in titles[i] or "微星 MPG GUNGNIR 110R WHITE" in titles[i] or "微星 MPG VELOX 100R WHITE" in titles[i]):
    sidePanel[i] = 0
  if("Antec P20CE" in titles[i] or "abee AS Enclosure RS07" in titles[i]):
    sfxPSU[i] = 1
